...
//                            val discoveryTask =
//                                project.tasks.create("trigger$discoveryClasspathConfigurationName.capitalize()", ScriptDefinitionDiscoveryTriggerTask::class.java) {
//                                    it.destinationDir = task.destinationDir
//                                    it.sourceSetName = task.sourceSetName
//                                }
//
//                            task.dependsOn(discoveryTask)
                        }
            ...
            if (kotlinSourceSet == null) {
                ...
                val extensions by lazy {
                    ...
                    if (discoveryResultsConfiguration == null) {
                        ...
                        val extensions = discoveryResultsConfiguration.files.flatMap {
                            it.readLines().filter(String::isNotBlank)
                        }.also {
                            kotlinSourceSet.addCustomSourceFilesExtensions(it)
...
open class ScriptDefinitionDiscoveryTriggerTask : DefaultTask() {

    @get:Internal
    var sourceSetName: String by Delegates.notNull()

    @get:Internal
    var destinationDir: File by Delegates.notNull()

    @get:OutputFile
    val output: File
        get() = destinationDir.resolve("discovery${sourceSetName.capitalize()}.ok")

    @Suppress("unused")
    @get:InputFiles
    val inputConfiguration: Set<File>
        get() = project.configurations.findByName(getDiscoveryResultsConfigurationName(sourceSetName))!!.files
